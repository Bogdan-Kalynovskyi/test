function QAgentEvents () {
    ...
    this.add = function (nodes) {
        ...
        for (var i = 0, n = nodes.length; i < n; i++) {
            ...
            for (var j in arr) {
                if (arr[j].time === event.time && arr[j].isLogged === event.isLogged) {
                    event = null;
                    break;
                }
            }
...
function QChart (container) {
    ...
    this.invalidate = function () {
        dbData = nonPieDataTable = undefined;
    };
...
function QDataBase (visibleCols, visibleRows) {
            ...
            if (withPercentage || time || loggedTime) {
                for (i in result) {
                    ...
                    if (withPercentage) {
                        var tblRow = table[i];
                        if (loggedTime) {
                            perc = tblRow[j0] ? Math.round(100 * tblRow[j0] / tblRow.totalTime) : '';
                        }
                        else {
    ...
    function byHours (period) {
        var now = Date.now() / 1000,
            startTime = START,
            normalizedStart = START - START % period,
